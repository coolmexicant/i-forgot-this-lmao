...
class PlayState extends MusicBeatState
{
	...
	public var camIntensity:Int = 50;
	...
	override public function create()
	{
		...
		pauseBlur = new BlurFilter(0, 0);
		pauseGlow = new GlowFilter(0, 0);
		camGame.filters = [pauseBlur, pauseGlow];
		camHUD.filters = [pauseBlur, pauseGlow];
		...
		if(stageData.camInt != null)
			camIntensity = stageData.camInt;
		...
		switch (curStage)
		{
			...
			case 'exesequel': new states.stages.DickShit();
		}
		...
		scoreTxt = new FlxText(0, healthBar.y + 40, FlxG.width, 'Points: 0 ${!instakillOnMiss ? ' • Combo Breaks: 0' : ''} • Acuraccy: 100% • Health: 50%', 20);
	...
	public function doScoreBop(badHit:Bool):Void {
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			FlxTween.tween(pauseBlur, {blurX: 0, blurY: 0}, 0.2, {ease: FlxEase.sineInOut});
			FlxTween.tween(pauseGlow, {blurX: 0, blurY: 0}, 0.2, {ease: FlxEase.sineInOut});
	...
	override public function update(elapsed:Float)
	{
		...
		shownAccuracy = CoolUtil.truncateFloat(FlxMath.lerp(shownAccuracy,ratingPercent*100, Main.adjustFPS(0.2)),2);
		if(ratingPercent<1 && shownAccuracy==100)shownAccuracy=99.99;
		if(Math.abs((ratingPercent*100)-shownAccuracy) <= 0.1)
			shownAccuracy=CoolUtil.truncateFloat(ratingPercent*100,2);
		shownScore = Math.floor(FlxMath.lerp(shownScore,songScore,Main.adjustFPS(0.2)));

		var tempScore:String = 'Points: $shownScore ${!instakillOnMiss ? ' • Combo Breaks: $songMisses' : ''} • Acuraccy: $shownAccuracy% • Health: ${CoolUtil.luaRound(health * 50, 0)}%';
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null){ // From Tr1ngle engine
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		if (iconP1.animation.curAnim.curFrame == 3)
			iconP1.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : (healthBar.percent < 20) ? 2 : 0;
		else if (iconP1.animation.curAnim.curFrame == 1)
			iconP1.animation.curAnim.curFrame = 0;
		else
			iconP1.animation.curAnim.curFrame = (healthBar.percent < 20) ? 1 : 0;

		if (iconP2.animation.curAnim.curFrame == 3)
	...
	function openPauseMenu()
	{
		...
		FlxTween.tween(pauseBlur, {blurX: 10, blurY: 10}, 0.2, {ease: FlxEase.sineInOut});
		FlxTween.tween(pauseGlow, {blurX: 10, blurY: 10}, 0.2, {ease: FlxEase.sineInOut});
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Camera Zoom Chain':
				...
				var gameZoom:Float = Std.parseFloat(split[0]);
				var hudZoom:Float = Std.parseFloat(split[1]);
				var shGame:Float = 0;
				var shHUD:Float = 0;
				var gameShake:Float = 0;
				var hudShake:Float = 0;
				var shakeTime:Bool = false;
				var toBeat:Float = Std.parseFloat(split2[0]);
				var tiBeat:Float = Std.parseFloat(split2[1]);
				var totalBeat:Float = 0;
				var timeBeat:Float = 0;
				if(!Math.isNaN(gameZoom))
					gameZoom = 0.015;
				if(!Math.isNaN(hudZoom))
					hudZoom = 0.03;
				if(split.length == 4){
					shGame = Std.parseFloat(split[2].trim());
					shHUD = Std.parseFloat(split[3].trim());
	...
	override function stepHit()
	{
		...
		Modchart.onStepHit(SONG.song.toLowerCase(), curStep);
